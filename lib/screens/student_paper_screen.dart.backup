...
class _StudentPaperScreenState extends State<StudentPaperScreen> {
  ...
  Future<void> _askAI(int idx) async {
    ...
    _toast('AI分析中，请稍候...');
    try {
      final res = await http.post(Uri.parse('http://127.0.0.1:8000/api/ai-hint'),headers: {'Content-Type': 'application/json'}, body: json.encode({'question': widget.questions[idx]['content'] ?? '', 'image': img}));
