...
class _StudentPaperScreenState extends State<StudentPaperScreen> {
  ...
  Future<void> _askAI(int idx) async {
    ...
    _toast('AI分析中，请稍候...');
    try {
      final res = await http.post(Uri.parse('http://192.168.1.104:8000/api/ai-hint'), headers: {'Content-Type': 'application/json'}, body: json.encode({'question': widget.questions[idx]['content'] ?? '', 'image': img}));
      if (res.statusCode == 200) {
        _safe(() => setState(() => _aiHints[idx] = json.decode(res.body)['hint']));
        _toast('✅ AI辅导已送达');
      }
    } catch (e) {}
  }
